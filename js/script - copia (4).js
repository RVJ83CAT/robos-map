...
function updateTable(data, page = 1) {
    ...
    let caseNumber = start + 1; // Número correlativo basado en el inicio de la página
    paginatedData.forEach((d, index) => {
        const globalIndex = start + index;
        const row = document.createElement('tr');
        row.innerHTML = `
            <td class="numero">${caseNumber++}</td>
...
function processExcelFile(file) {
    ...
    reader.onload = function(e) {
        try {
            ...
            // Skip the first column ("N°") and take the next 16 columns for comparison
            const headers = rows[0].slice(1, CONFIG.EXPECTED_COLUMNS.length + 1).map(normalizeColumnName);
            const expectedNormalized = CONFIG.EXPECTED_COLUMNS.map(normalizeColumnName);
            console.log('Encabezados detectados (sin N°):', headers);
            console.log('Encabezados esperados:', expectedNormalized);

            // Validate headers
            const headersMatch = headers.every((header, index) => header === expectedNormalized[index]);
            if (!headersMatch) {
                throw new Error('Los encabezados del archivo no coinciden con los esperados. Asegúrese de que las columnas sean: ' + CONFIG.EXPECTED_COLUMNS.join(', '));
            }
            ...
            // Process rows, skipping the first column ("N°")
            for (let i = 1; i < rows.length; i++) {
                const row = rows[i].slice(1, CONFIG.EXPECTED_COLUMNS.length + 1); // Skip N° column
                if (!isRowEmpty(row)) {
                    const data = {};
                    ...
                    if (isNaN(lat) || isNaN(lng)) {
                        console.warn(`Coordenadas inválidas en fila ${i + 1}: LATITUD=${data.LATITUD}, LONGITUD=${data.LONGITUD}`);
            ...
            if (allData.length === 0) {
                throw new Error('No se encontraron datos válidos con coordenadas en el archivo.');
            }
